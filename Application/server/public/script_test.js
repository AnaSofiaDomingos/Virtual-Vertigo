...
window.onload = function () {
    ...
    // function who permit to start the movements
    // NOT USED ANY MORE
    function startMoving(moyenne){
        for (var i = 0; i < moyenne.length; i++) {

            // Move the character in fonction of the head
            if (moyenne[i].name == "head"){
				if ((moyenne[i].z > 3.93) && !start)
					start = true; 
            }
	...
	function positionning(id, pos){
		var member = $("#"+id);
		member.attr("translation", (pos.x) + " " +  (pos.y -0.5) + " " + pos.z);
	...
	function scaling(id, distance, old){
		...
		if (distance != old){
			member.attr("scale", distance  +" "+ DIV_DISTANCE + " " + DIV_DISTANCE );
			old = distance;
		}
	...
	function anime(id, angle, old){
		...
		if (angle != old){
			member.attr("rotation", "0 0 1 " + angle);
			old = angle;
		}
	}
	...
	// move the character in fonction of the position of the head
    // NOT USED ANY MORE
	...
	// move the animated character in fonction of the head
	function move(){
	...
	// function to check if the character is out of the board
	function checkBoundries(){
		var positionCharacter = $("#armature").attr("translation").split(" ");

		var posLegL = $("#footleft").attr("translation").split(" ");
		var posLegR = $("#footright").attr("translation").split(" ");

		// personnage non animé centré : 4.96 - personnage totalement en dehors g :  3.26	d: 7.32 
		//if ((positionCharacter[2] <= 3.26) || (positionCharacter[2] >= 7.32)) 
		//	retry()
	...
	function died(){
		...
		setTimeout(function(){clearInterval(fall);}, 300 * 7);
	}
